...
begin
  ...
  if found and submitter_is_admin and (result.review_status <> 'approved' or result.promoted_price_id is null) then
    ...
    insert into public.user_price_logs (user_id, product_id, store_id, price_yen, purchased_at, note, share_to_public, review_status, confidence_score, review_note, reviewed_at, promoted_price_id, evidence_url)
    values (
      auth.uid(),
      target_product_id,
      target_store_id,
