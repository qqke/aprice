    ...
    return `
      ...
      >
        <div class="feed__copy">
          <strong>${item.name || 'Unknown store'}</strong>
          <small>${metaBits.filter(Boolean).join(' · ')}</small>
        </div>
  ...
  priceList.innerHTML = rows.map((row, index) => {
    ...
    return index === 0 ? `
      <a class="feed__item feed__item--featured" href="${priceHref}">
        <div class="feed__copy">
          ...
          <small>${row.stores?.city || ''}${distanceLabel}${row.stores?.hours ? ` · ${row.stores.hours}` : ''}</small>
        </div>
        <div class="feed__meta">
    ...
    ` : `
      <a class="feed__item" href="${priceHref}">
        <div>
          <strong>${row.stores?.name || 'Unknown store'}</strong>
          <small>${row.stores?.city || ''}${distanceLabel}${row.is_member_price ? ' · 会员价' : ' · 公开价'}${row.stores?.hours ? ` · ${row.stores.hours}` : ''}</small>
        </div>
        <div class="feed__meta">
    ...
    ? rows.map((row, index) => index === 0 ? `
      <div class="feed__item feed__item--featured">
        <div class="feed__copy">
          ...
          <strong>${row.stores?.name || 'Unknown store'}</strong>
          <small>${row.stores?.city || ''}${row.stores?.pref ? ` · ${row.stores.pref}` : ''}${row.stores?.hours ? ` · ${row.stores.hours}` : ''}</small>
        </div>
        <div class="feed__meta">
          <div class="price">¥${row.price_yen}</div>
          <span class="pill">${row.is_member_price ? '会员价' : '公开价'}</span>
          <button class="button button--ghost product-nearby__favorite" type="button" data-favorite-store="${row.store_id}">收藏</button>
        </div>
      </div>
    ` : `
      <div class="feed__item">
